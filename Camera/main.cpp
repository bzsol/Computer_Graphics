...
int main() {
    ...
    //GLfloat matrix[] = {
    //  cos(theta), -sin(theta), 0.0f, 0.0f, // first column
    //  0.0f, 1.0f, 0.0f, 0.0f, // second column
    //  sin(theta), cos(theta), 1.0f, 0.0f, // third column
    ...
    int matrix2_location = glGetUniformLocation(shader_programme, "matrix2");
    glUseProgram(shader_programme);
    glUniformMatrix4fv(matrix2_location, 1, GL_FALSE, matrix2.m);
    ...
    while (!glfwWindowShouldClose(g_window)) {
        ...
        /*  matrix[0] = cos(theta);
          ...
          matrix[9] = cos(theta);
          last_position = theta;
          if (fabs(theta) > 1.0) { speed = -speed; }

          glUniformMatrix4fv(matrix_location, 1, GL_FALSE, matrix);*/

        if (fabs(theta) > 1.0) { speed = -speed; }

        matrix2 = rotate_y_deg(identity_mat4(), theta * 100.0f);
